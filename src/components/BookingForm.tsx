...
const BookingForm: React.FC = () => {
  ...
  const formatCityLabel = (city: any) => {
    ...
    const stateAbbreviation = city.state 
      ? city.state.length > 2 
    ...
    <div className="w-full bg-[#FEF7E4] rounded-lg md:rounded-2xl shadow-lg overflow-hidden">
      <div className="pt-5 md:pt-7 pb-6 md:pb-8 bg-gradient-to-b from-amber-300 to-amber-200 py-0 px-[20px] md:px-[54px] bg-amber-500">
        ...
        <div className="space-y-5 md:space-y-6">
          ...
          {distanceInfo && (
            <div className="bg-white rounded-lg p-3 text-sm border border-amber-200">
              <div className="flex items-center justify-between">
                <span className="font-medium text-gray-700">Distância estimada:</span>
                <span className="font-bold text-amber-600">{Math.round(distanceInfo.distance / 1000)} km</span>
              </div>
              <div className="flex items-center justify-between mt-1">
                <span className="font-medium text-gray-700">Tempo estimado:</span>
                <span className="font-bold text-amber-600">{Math.round(distanceInfo.duration / 60)} minutos</span>
              </div>
            </div>
          )}
