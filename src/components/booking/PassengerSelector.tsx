    ...
    <TransitionEffect direction="fade" delay={250}>
      <div className="space-y-2">
        ...
        <Select value={value} onValueChange={handleValueChange} open={open} onOpenChange={setOpen}>
          <SelectTrigger 
            className="w-full py-5 md:py-6 rounded-lg border border-gray-200/30 shadow-sm 
                     bg-white/95 focus:border-amber-300 focus:ring-amber-300 text-gray-700 
                     transition-all duration-200 hover:border-amber-200 hover:bg-white"
          >
          ...
          <SelectContent className="max-h-[200px] bg-white shadow-lg border border-gray-200 z-50">
            {passengerOptions.map(num => (
              <SelectItem 
                key={num} 
                value={num} 
                className="cursor-pointer hover:bg-amber-50 py-3 transition-colors flex items-center"
              >
